...
sem_open(int sem, int value)
{
  ...
  for (aux_sem = sems; aux_sem < &sems[NSEM]; aux_sem++) {
    if (aux_sem->value == -1 && !free_space) {		// si el semaforo esta cerrado y todavia no se encontro uno libre
      free_space = true;						// si hay algun semaforo libre
	  set_sem = aux_sem;
	}
	if (aux_sem->value != -1 && aux_sem->sem_id == sem){ // si el semaforo estaba en uso veo si no tiene el mismo id que 
		unused_id = false;
	}
  }
  ...
  if (!unused_id) 				// si hay un semaforo con el mismo id
  {
	printf("sem was alredy create with that id\n");
    release(&general_lock);
    return 0;						// devuelve 0 para decir que hubo un error
  }
  ...
  set_sem->sem_id = sem;
  set_sem->value = value;
  initlock(&set_sem->lock, "sem lock");
...
sem_up(int sem)
{
  ...
  if (!exists_id) {
    printf("That semaphore id doesn't exists %d \n",sem);
    release(&general_lock);
    return 0;

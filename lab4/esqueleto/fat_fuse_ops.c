...
static void fat_fuse_log_activity(char *operation_type, fat_file file) {
    ...
    now_to_str(buf);                 // guarda la hora en buf
    strcat(buf, "\t");               // strcat concatena dos strings
    strcat(buf, getlogin());         // nombre de usuario 
    strcat(buf, "\t");
    strcat(buf, file->filepath);     // agrega el PATH
    strcat(buf, "\t");
    strcat(buf, operation_type);     // read o write
    strcat(buf, "\n");
    int message_size = strlen(buf);  // largo de caracteres del mensaje

    fat_volume vol = get_fat_volume();
    fat_tree_node log = fat_tree_node_search(vol->file_tree,"/fs.log");
    fat_file logfile = fat_tree_get_file(log);
    fat_file parent = fat_tree_get_parent(log);

    fat_file_pwrite(logfile, buf, message_size, logfile->dentry->file_size, parent);
}
...
int fat_fuse_read(const char *path, char *buf, size_t size, off_t offset,
    ...
    if (!is_created) {  // if it was created after mounting
        fat_volume vol = get_fat_volume();
        if (fat_tree_search(vol->file_tree,"/fs.log")==NULL) {  // if it was created before mounting
            fat_fuse_mknod("/fs.log",0,0);
        }
        is_created = 1;
    }

    if (strcmp(path,"/fs.log")!=0)
